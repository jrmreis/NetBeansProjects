...
procedure TForm1.SpinEdit1Change(Sender: TObject);
 var j,n,y : integer;       //respect. colunas ordem da matriz e valor numérico do X ex: X1

      begin

n:=spinedit1.value;  //colocando o valor do spinedit no "n"

if n>1 then           //condição para config. do string grid
...
begin                                            //botão para fechar o programa
CLOSE;
end;

procedure TForm1.Button2Click(Sender: TObject);
var n,j, i:integer;                              //botão para limpar todos os
begin                                            //strings grids
n:=spinedit1.value;
for i:=1 to n do
   for j:=0 to n+1 do
...
for j:=0 to n do                        //for especial para o string Grid3
    stringgrid3.cells[j,1]:='';         //pois ele só tem uma linha
end;
procedure TForm1.Button1Click(Sender: TObject);  //Botão resolver
var                                        
a: array[1..20,1..20] of Double;   //matriz a
i, j, xi, xj, n: integer;   //variáveis aux. das posições da matriz
begin
n:=spinedit1.value;
...
i:=1;
j:=1;                                //inicializando variáveis
while (i<n) and (j<=n+1) do
      begin                           //bloco principal
         for xi:=i to n do            //looping para andar pela matriz
            for xj:=j to n+1 do       //com variaáveis auxiliares fazer o efeito "comer espiga de milho"
     ...
     for i:=1 to n do             //novo looping para não confundir com o da transf. de srt p/ float (este é o inverso, claro!)
        for j:=1 to n+1 do
...
i, j, n: integer;     //variáveis de posição
s: Double;        //variável da somatória
begin
n:=spinedit1.value;
for i:=1 to n do
   for j:=0 to n do
      a[i,j+1]:=strtofloat(stringgrid2.cells[j,i]); //transformando em matriz
      r[n]:=a[n,n]/a[n,n+1];      //responsável pelo elemento1 da retro equação
   for i:=n downto 1 do             //comando de retro equação do maior para o menor
       begin
          s:=0;                 //inicializando somatória
          for j:=i+1 to n do         //responsável pelo elemento2 da retro equação
             s:=s+(a[i,j]*r[j]);                       //somatória que faz parte da retroequação
             r[i]:=(a[i,n+1]-s)/a[i,i];                    //formula da retro equação
             stringgrid3.cells[i-1,1]:=floattostr(r[i]);      //colocando na string3
       end;

end;

procedure TForm1.Button4Click(Sender: TObject);  //botão voltar
begin
tabbedNotebook1.pageindex:=0;
end;

procedure TForm1.Button6Click(Sender: TObject);  //botão teste1
...
procedure TForm1.Button7Click(Sender: TObject);    //botão teste2
...
procedure TForm1.Button8Click(Sender: TObject);    //botão teste3
begin
spinedit1.value:=10;    //definindo tamanho da string grid
